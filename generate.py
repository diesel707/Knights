...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for X in self.domains[x]:
            for Y in self.domains[y]:
                if X[list(self.crossword.overlaps[x, y])[0]] == Y[list(self.crossword.overlaps[x, y])[1]]:
                    break
            else:
                remove.append(X)
                rev = True
    ...
    def consistent(self, assignment):
        ...
        for x, y in assignment.items():
            if y in assignments or x.length != len(y):
                return False
            assignments.append(y)

            for neighbor in self.crossword.neighbors(x):
                if neighbor in assignment and assignment[neighbor][list(self.crossword.overlaps[x, neighbor])[1]] != y[list(self.crossword.overlaps[x, neighbor])[0]]:
                    return False
        return True
        # raise NotImplementedError
            ...
            if i in assignment:
                ...
                for j in self.crossword.neighbors(var):
                    if i in self.domains[j]:
                        xy += 1
                d[i] = xy
        return sorted(d, key = lambda sort: d[sort])
        # raise NotImplementedError
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for i in self.domains:
            if i in assignment:
                ...
                if mrv > len(self.domains[i]):
                    x1 = i
                    mrv = len(self.domains[i])
                    if self.crossword.neighbors(i) == None:
                        deg = 0
                    else:
                        deg = len(self.crossword.neighbors(i))
                elif mrv == len(self.domains[i]):
                    if self.crossword.neighbors(i) != None:
